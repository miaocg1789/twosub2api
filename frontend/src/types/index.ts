...
export type GroupPlatform = 'anthropic' | 'openai' | 'gemini' | 'antigravity' | 'sora' | 'deepseek' | 'qwen' | 'glm' | 'kimi' | 'iflow' | 'kiro'
...
export interface AdminGroup extends Group {
  ...
  // 按次计费配置
  per_request_pricing: Record<string, number> | null
...
export type AccountPlatform = 'anthropic' | 'openai' | 'gemini' | 'antigravity' | 'sora' | 'deepseek' | 'qwen' | 'glm' | 'kimi' | 'iflow' | 'kiro'
...
export interface Account {
  ...
  daily_used_cost?: number | null
  daily_used_requests?: number | null
}
...
// Kiro 单个模型的日使用量
export interface KiroModelUsage {
  requests: number // 日请求数
  tokens: number   // 日 Token 数
  cost: number     // 日费用
}

// Kiro 订阅额度信息（从 CodeWhisperer API 获取）
export interface KiroSubscriptionQuota {
  subscription_type: string   // FREE/PRO/PRO_PLUS/POWER
  current_usage: number       // 当前使用量
  usage_limit: number         // 总额度
  usage_percent: number       // 使用百分比 (0-100)
  next_reset_date: string     // 重置日期 YYYY-MM-DD
  trial_status?: string       // 试用状态
  trial_current?: number      // 试用已用
  trial_limit?: number        // 试用总量
  email?: string              // 邮箱
}
...
export interface AccountUsageInfo {
  ...
  kiro_quota?: Record<string, KiroModelUsage> | null
...
export interface PaymentOrder {
  ...
  amount: number
  credit: number
  payment_method: 'alipay' | 'alipay_f2f' | 'wechat' | 'epay'
