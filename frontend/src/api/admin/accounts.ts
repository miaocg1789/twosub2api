...
/**
 * Query upstream balance for an account
 * @param id - Account ID
...
/**
 * Batch query upstream balance for multiple accounts
 * @param accountIds - Array of account IDs
 * @returns Batch result with success/failed counts and per-account balance info
 */
export async function batchGetBalance(accountIds: number[]): Promise<{
  ...
  const { data } = await apiClient.post('/admin/accounts/batch-balance', { account_ids: accountIds })
  return data
}

/**
 * Fetch available models from upstream API for an account
 * @param id - Account ID
 * @returns List of upstream models
 */
export async function fetchUpstreamModels(id: number): Promise<Array<{
  id: string
  display_name?: string
}>> {
  const { data } = await apiClient.get(`/admin/accounts/${id}/upstream-models`)
  return data
}
