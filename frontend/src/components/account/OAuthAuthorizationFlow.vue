<template>
  ...
  >
      ...
      <div class="flex-1">
        ...
        <!-- Qwen Device Flow -->
        <div v-if="isQwen && inputMethod === 'manual'" class="space-y-4">
          <p class="mb-4 text-sm text-blue-800 dark:text-blue-300">
            {{ t('admin.accounts.oauth.qwen.followSteps') }}
          </p>
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ t('admin.accounts.oauth.qwen.step1InitiateFlow') }}
                </p>
                <button
                  v-if="!qwenDeviceFlowInfo"
                  type="button"
                  :disabled="loading"
                  class="btn btn-primary text-sm"
                  @click="emit('initiate-device-flow')"
                >
                  <svg
                    v-if="loading"
                    class="-ml-1 mr-2 h-4 w-4 animate-spin"
                  ...
                  {{ t('admin.accounts.oauth.qwen.initiateDeviceFlow') }}
                </button>
                <div v-else class="rounded-lg bg-green-50 p-3 dark:bg-green-900/20">
                  <p class="text-sm text-green-700 dark:text-green-300">
                    <Icon name="check" size="sm" class="mr-1 inline text-green-600" />
                    {{ t('admin.accounts.oauth.qwen.deviceFlowInitiated') }}
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ t('admin.accounts.oauth.qwen.step2VisitUrl') }}
                </p>
                <div class="space-y-3">
                  <div class="flex items-center gap-2">
                    <a
                      :href="qwenDeviceFlowInfo.verification_url_complete || qwenDeviceFlowInfo.verification_url"
                      target="_blank"
                      rel="noopener noreferrer"
                      class="btn btn-secondary flex-1 justify-center text-sm"
                    >
                      <Icon name="link" size="sm" class="mr-2" />
                      {{ t('admin.accounts.oauth.qwen.openVerificationUrl') }}
                    </a>
                    <button
                      type="button"
                      class="btn btn-secondary p-2"
                      title="Copy URL"
                      @click="handleCopyQwenUrl"
                    >
                      <svg
                        v-if="!copied"
                        class="h-4 w-4"
                  ...
                  <div
                    v-if="qwenDeviceFlowInfo.user_code"
                    class="rounded-lg bg-gray-50 p-3 dark:bg-gray-700"
                  >
                    <p class="mb-1 text-xs text-gray-500 dark:text-gray-400">
                      {{ t('admin.accounts.oauth.qwen.userCode') }}
                    </p>
                    <p class="text-center font-mono text-2xl font-bold tracking-widest text-blue-700 dark:text-blue-300">
                      {{ qwenDeviceFlowInfo.user_code }}
                    </p>
                  </div>
                </div>
              </div>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ t('admin.accounts.oauth.qwen.step3WaitAuth') }}
                </p>
                <div v-if="qwenPollingStatus === 'polling'" class="flex items-center gap-2">
                  <svg
                  ...
                  <span class="text-sm text-blue-700 dark:text-blue-300">
                    {{ t('admin.accounts.oauth.qwen.polling') }}
                  </span>
                </div>
                <div v-else-if="qwenPollingStatus === 'success'" class="flex items-center gap-2">
                  <Icon name="check" size="sm" class="text-green-600" />
                  <span class="text-sm font-medium text-green-700 dark:text-green-300">
                    {{ t('admin.accounts.oauth.qwen.authSuccess') }}
                  </span>
                </div>
                <button
                  v-else
                  ...
                  :disabled="loading"
                  class="btn btn-primary text-sm"
                  @click="emit('poll-token')"
                >
                  <Icon name="refresh" size="sm" class="mr-2" />
                  {{ t('admin.accounts.oauth.qwen.startPolling') }}
                </button>
              </div>
            </div>
          </div>
        ...
        <!-- Kimi Device Flow -->
        <div v-if="isKimi && inputMethod === 'manual'" class="space-y-4">
          <p class="mb-4 text-sm text-blue-800 dark:text-blue-300">
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ t('admin.accounts.oauth.kimi.step1InitiateFlow') }}
                </p>
                <button
                  v-if="!kimiDeviceFlowInfo"
                  type="button"
                  :disabled="loading"
                  class="btn btn-primary text-sm"
                  @click="emit('kimi-initiate-device-flow')"
                >
                  <svg
                    v-if="loading"
                    class="-ml-1 mr-2 h-4 w-4 animate-spin"
                  ...
                  <Icon v-else name="link" size="sm" class="mr-2" />
                  {{ t('admin.accounts.oauth.kimi.initiateDeviceFlow') }}
                </button>
                <div v-else class="rounded-lg bg-green-50 p-3 dark:bg-green-900/20">
                  <p class="text-sm text-green-700 dark:text-green-300">
                    <Icon name="check" size="sm" class="mr-1 inline text-green-600" />
                    {{ t('admin.accounts.oauth.kimi.deviceFlowInitiated') }}
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ t('admin.accounts.oauth.kimi.step2VisitUrl') }}
                </p>
                <div class="space-y-3">
                  <div class="flex items-center gap-2">
                    <a
                      :href="kimiDeviceFlowInfo.verification_url_complete || kimiDeviceFlowInfo.verification_url"
                      target="_blank"
                      rel="noopener noreferrer"
                      class="btn btn-secondary flex-1 justify-center text-sm"
                    >
                      <Icon name="link" size="sm" class="mr-2" />
                      {{ t('admin.accounts.oauth.kimi.openVerificationUrl') }}
                    </a>
                    <button
                      type="button"
                      class="btn btn-secondary p-2"
                      title="Copy URL"
                      @click="handleCopyKimiUrl"
                    >
                      <svg
                        v-if="!copied"
                        class="h-4 w-4"
                  ...
                  <div v-if="kimiDeviceFlowInfo.user_code" class="rounded-lg bg-gray-50 p-3 dark:bg-gray-700">
                    <p class="mb-1 text-xs text-gray-500 dark:text-gray-400">
                      {{ t('admin.accounts.oauth.kimi.userCode') }}
                    </p>
                    <p class="text-center font-mono text-2xl font-bold tracking-widest text-blue-700 dark:text-blue-300">
                      {{ kimiDeviceFlowInfo.user_code }}
                    </p>
                  </div>
                </div>
              </div>
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <p class="mb-2 font-medium text-blue-900 dark:text-blue-200">
                  {{ t('admin.accounts.oauth.kimi.step3WaitAuth') }}
                </p>
                <div v-if="kimiPollingStatus === 'polling'" class="flex items-center gap-2">
                  <svg class="h-5 w-5 animate-spin text-blue-500" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
                  <span class="text-sm text-blue-700 dark:text-blue-300">
                    {{ t('admin.accounts.oauth.kimi.polling') }}
                  </span>
                </div>
                <div v-else-if="kimiPollingStatus === 'success'" class="flex items-center gap-2">
                  <Icon name="check" size="sm" class="text-green-600" />
                  <span class="text-sm font-medium text-green-700 dark:text-green-300">
                    {{ t('admin.accounts.oauth.kimi.authSuccess') }}
                  </span>
                </div>
                <button
                  v-else
                  ...
                  :disabled="loading"
                  class="btn btn-primary text-sm"
                  @click="emit('kimi-poll-token')"
                >
                  <Icon name="refresh" size="sm" class="mr-2" />
                  {{ t('admin.accounts.oauth.kimi.startPolling') }}
                </button>
              </div>
            </div>
          </div>
        ...
        <!-- iFlow Cookie Auth -->
        <div v-if="isIFlow && inputMethod === 'manual'" class="space-y-4">
          <div
            class="rounded-lg border border-blue-300 bg-white/80 p-4 dark:border-blue-600 dark:bg-gray-800/80"
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{ t('admin.accounts.oauth.iflow.cookieAuthDesc') }}
            </p>
            ...
            <div class="mb-4">
              ...
              <textarea
                v-model="iflowCookieInput"
                rows="3"
                class="input w-full resize-y font-mono text-sm"
                :placeholder="t('admin.accounts.oauth.iflow.cookiePlaceholder')"
              ></textarea>
            </div>
            ...
            <button
              type="button"
              class="btn btn-primary w-full"
              :disabled="loading || !iflowCookieInput.trim()"
              @click="handleIFlowCookieAuth"
            >
              <svg
                v-if="loading"
                class="-ml-1 mr-2 h-4 w-4 animate-spin"
              ...
              {{
                loading
                  ? t('admin.accounts.oauth.iflow.authenticating')
                  : t('admin.accounts.oauth.iflow.authenticate')
              }}
            </button>
          </div>
        </div>
        ...
        <!-- Manual Authorization Flow -->
        <div v-if="inputMethod === 'manual' && !isQwen && !isKimi && !isIFlow" class="space-y-4">
          <p class="mb-4 text-sm text-blue-800 dark:text-blue-300">
            {{ oauthFollowSteps }}
          </p>
...
interface Props {
  ...
  // Qwen device flow props
  qwenDeviceFlowInfo?: { user_code?: string; verification_url?: string; verification_url_complete?: string; expires_in?: number; interval?: number } | null
  qwenPollingStatus?: 'idle' | 'initiated' | 'polling' | 'success' | 'error' | 'expired'
  // Kimi device flow props
  kimiDeviceFlowInfo?: { user_code?: string; verification_url?: string; verification_url_complete?: string; expires_in?: number; interval?: number } | null
  kimiPollingStatus?: 'idle' | 'initiated' | 'polling' | 'success' | 'error' | 'expired'
}
...
const props = withDefaults(defineProps<Props>(), {
  ...
  platform: 'anthropic',
  showProjectId: true,
  qwenDeviceFlowInfo: null,
  qwenPollingStatus: 'idle',
  kimiDeviceFlowInfo: null,
  kimiPollingStatus: 'idle'
})
...
const emit = defineEmits<{
  ...
  'poll-token': []
  'kimi-initiate-device-flow': []
  'kimi-poll-token': []
  'iflow-cookie-auth': [cookie: string]
}>()

const { t } = useI18n()

const isOpenAI = computed(() => props.platform === 'openai' || props.platform === 'sora')
const isQwen = computed(() => props.platform === 'qwen')
const isKimi = computed(() => props.platform === 'kimi')
const isIFlow = computed(() => props.platform === 'iflow')
...
const getOAuthKey = (key: string) => {
  ...
  if (props.platform === 'qwen') return `admin.accounts.oauth.qwen.${key}`
  if (props.platform === 'kimi') return `admin.accounts.oauth.kimi.${key}`
  if (props.platform === 'iflow') return `admin.accounts.oauth.iflow.${key}`
...
const handleCopyQwenUrl = () => {
  if (props.qwenDeviceFlowInfo) {
    const url = props.qwenDeviceFlowInfo.verification_url_complete || props.qwenDeviceFlowInfo.verification_url || ''
    copyToClipboard(url, 'URL copied to clipboard')
  }
}

const handleCopyKimiUrl = () => {
  if (props.kimiDeviceFlowInfo) {
    const url = props.kimiDeviceFlowInfo.verification_url_complete || props.kimiDeviceFlowInfo.verification_url || ''
    copyToClipboard(url, 'URL copied to clipboard')
  }
}

const handleIFlowCookieAuth = () => {
  if (iflowCookieInput.value.trim()) {
    emit('iflow-cookie-auth', iflowCookieInput.value)
  }
}
