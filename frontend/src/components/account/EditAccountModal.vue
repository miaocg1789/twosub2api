<template>
  ...
  >
    ...
    >
          ...
          <input
            ...
            :placeholder="
              account.platform === 'openai' || account.platform === 'sora'
                ...
                : account.platform === 'gemini'
                  ? 'AIza...'
                  : account.platform === 'antigravity' || account.platform === 'deepseek' || account.platform === 'qwen' || account.platform === 'glm'
                    ? 'sk-...'
                    : 'sk-ant-...'
            "
          />
        ...
        <div v-if="account.platform !== 'antigravity'" class="border-t border-gray-200 pt-4 dark:border-dark-600">
          <div class="mb-2 flex items-center justify-between">
            ...
            <button
              ...
              class="text-xs text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {{ isFetchingModels ? t('admin.accounts.fetchingUpstreamModels') : t('admin.accounts.fetchUpstreamModels') }}
            </button>
          </div>
      ...
      <!-- Model Restriction Section for Kiro OAuth -->
      <div
      ...
      >
        <div class="mb-2 flex items-center justify-between">
          ...
          <button
            ...
            @click="handleFetchUpstreamModels"
            :disabled="isFetchingModels"
            class="text-xs text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {{ isFetchingModels ? t('admin.accounts.fetchingUpstreamModels') : t('admin.accounts.fetchUpstreamModels') }}
          </button>
        </div>
        ...
        <div v-if="modelRestrictionMode === 'whitelist'">
          <ModelWhitelistSelector v-model="allowedModels" :platform="account?.platform || 'kiro'" />
          <p class="text-xs text-gray-500 dark:text-gray-400">
            {{ t('admin.accounts.selectedModels', { count: allowedModels.length }) }}
...
const defaultBaseUrl = computed(() => {
  ...
  if (props.account?.platform === 'qwen') return 'https://dashscope.aliyuncs.com/compatible-mode'
  if (props.account?.platform === 'glm') return 'https://open.bigmodel.cn/api/paas'
...
watch(
  ...
  (newAccount) => {
    if (newAccount) {
      ...
      // Load daily limits settings (all platforms)
      loadDailyLimitsSettings(newAccount)
      ...
      if (newAccount.type === 'apikey' && newAccount.credentials) {
        ...
        // Load model mappings for oauth/setup-token accounts (e.g. Kiro)
        const oauthCredentials = newAccount.credentials as Record<string, unknown> | undefined
        const existingOauthMappings = oauthCredentials?.model_mapping as Record<string, string> | undefined
        if (existingOauthMappings && typeof existingOauthMappings === 'object') {
          const entries = Object.entries(existingOauthMappings)
          const isWhitelistMode = entries.length > 0 && entries.every(([from, to]) => from === to)
          if (isWhitelistMode) {
            modelRestrictionMode.value = 'whitelist'
            allowedModels.value = entries.map(([from]) => from)
...
// Load daily limits from account (all platforms)
function loadDailyLimitsSettings(account: Account) {
  dailyCostLimitEnabled.value = false
  dailyCostLimit.value = null
  dailyRequestLimitEnabled.value = false
  dailyRequestLimit.value = null

  const extra = account.extra as Record<string, unknown> | undefined
  if (!extra) return

  const costLimit = extra.daily_cost_limit as number | undefined
  if (costLimit != null && costLimit > 0) {
    dailyCostLimitEnabled.value = true
    dailyCostLimit.value = costLimit
  }

  const requestLimit = extra.daily_request_limit as number | undefined
  if (requestLimit != null && requestLimit > 0) {
    dailyRequestLimitEnabled.value = true
    dailyRequestLimit.value = requestLimit
  }
}
...
const handleFetchUpstreamModels = async () => {
  ...
  try {
    const models = (props.account.platform === 'kiro' && props.account.type !== 'apikey')
      ? await adminAPI.accounts.getAvailableModels(props.account.id)
      : await adminAPI.accounts.fetchUpstreamModels(props.account.id)

    if (modelRestrictionMode.value === 'whitelist') {
      // Add fetched models to whitelist
      const newModels = models.map(m => m.id).filter(id => !allowedModels.value.includes(id))
      if (newModels.length > 0) {
        allowedModels.value = [...allowedModels.value, ...newModels]
        appStore.showSuccess(t('admin.accounts.upstreamModelsFetched', { count: models.length }))
      } else {
        appStore.showInfo(t('admin.accounts.upstreamModelsFetched', { count: models.length }))
      }
    } else {
      // Mapping mode - just show info
      appStore.showInfo(t('admin.accounts.upstreamModelsFetched', { count: models.length }))
    }
  } catch (error: any) {
    appStore.showError(error.response?.data?.message || error.response?.data?.detail || t('admin.accounts.upstreamModelsFetchError'))
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      ...
      // Save model mapping for oauth accounts (e.g. Kiro)
    ...
    // Daily limits (all platforms) - merge into existing extra
    {
      ...
      if (dailyRequestLimitEnabled.value && dailyRequestLimit.value != null && dailyRequestLimit.value > 0) {
        newExtra.daily_request_limit = dailyRequestLimit.value
      } else {
        delete newExtra.daily_request_limit
      }
      updatePayload.extra = newExtra
    }
