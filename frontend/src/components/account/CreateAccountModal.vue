<template>
  ...
  >
    ...
    >
      ...
      <!-- Account Type Info (DeepSeek/GLM - API Key only) -->
      <div v-if="form.platform === 'deepseek' || form.platform === 'glm'">
        <label class="input-label">{{ t('admin.accounts.accountType') }}</label>
        <div class="mt-2 rounded-lg border-2 border-sky-500 bg-sky-50 p-3 dark:bg-sky-900/20">
          <div class="flex items-center gap-3">
            <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-sky-500 text-white">
              <Icon name="key" size="sm" />
            </div>
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">API Key</span>
      ...
      <!-- Qwen Account Type Selection (OAuth or API Key) -->
      <div v-if="form.platform === 'qwen'">
        <label class="input-label">{{ t('admin.accounts.accountType') }}</label>
        <div class="mt-2 grid grid-cols-2 gap-3">
          <button
            type="button"
            @click="qwenAccountType = 'oauth'; accountCategory = 'oauth-based'"
            :class="[
              'flex items-center gap-3 rounded-lg border-2 p-3 text-left transition-all',
              qwenAccountType === 'oauth'
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">{{ t('admin.accounts.oauth.qwen.accountTypeOAuth') }}</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.oauth.qwen.accountTypeOAuthDesc') }}</span>
            </div>
          </button>
          <button
            type="button"
            @click="qwenAccountType = 'apikey'; accountCategory = 'apikey'"
            :class="[
              'flex items-center gap-3 rounded-lg border-2 p-3 text-left transition-all',
              qwenAccountType === 'apikey'
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">{{ t('admin.accounts.oauth.qwen.accountTypeApiKey') }}</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.oauth.qwen.accountTypeApiKeyDesc') }}</span>
            </div>
          </button>
        </div>
      </div>

      <!-- Account Type Info (Kimi - OAuth Device Flow only) -->
      <div v-if="form.platform === 'kimi'">
        <label class="input-label">{{ t('admin.accounts.accountType') }}</label>
        <div class="mt-2 rounded-lg border-2 border-indigo-500 bg-indigo-50 p-3 dark:bg-indigo-900/20">
          <div class="flex items-center gap-3">
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">OAuth</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.oauth.kimi.title') }}</span>
            </div>
          </div>
        </div>
      </div>

      <!-- Account Type Info (iFlow - Cookie Auth only) -->
      <div v-if="form.platform === 'iflow'">
        <label class="input-label">{{ t('admin.accounts.accountType') }}</label>
        <div class="mt-2 rounded-lg border-2 border-cyan-500 bg-cyan-50 p-3 dark:bg-cyan-900/20">
          <div class="flex items-center gap-3">
            <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-cyan-500 text-white">
              <Icon name="key" size="sm" />
            </div>
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">Cookie</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.oauth.iflow.cookieAuth') }}</span>
            </div>
          </div>
        </div>
      </div>

      <!-- Account Type Info (Kiro - Refresh Token only) -->
      <div v-if="form.platform === 'kiro'">
        <label class="input-label">{{ t('admin.accounts.accountType') }}</label>
        <div class="mt-2 rounded-lg border-2 border-rose-500 bg-rose-50 p-3 dark:bg-rose-900/20">
          <div class="flex items-center gap-3">
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">OAuth</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.oauth.kiro.title') }}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <input
            ...
            :placeholder="
              form.platform === 'openai'
                ...
                : form.platform === 'gemini'
                  ? 'AIza...'
                  : form.platform === 'deepseek' || form.platform === 'qwen' || form.platform === 'glm'
                    ? 'sk-...'
                    : 'sk-ant-...'
            "
          />
      ...
      <!-- Daily Limits (all platforms) -->
      <div class="border-t border-gray-200 pt-4 dark:border-dark-600 space-y-4">
        <div class="mb-3">
          <h3 class="input-label mb-0 text-base font-semibold">{{ t('admin.accounts.dailyLimits.title') }}</h3>
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            {{ t('admin.accounts.dailyLimits.hint') }}
          </p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
            <div class="mb-3 flex items-center justify-between">
              <label class="input-label mb-0">{{ t('admin.accounts.dailyLimits.cost.label') }}</label>
              <button
                type="button"
                @click="dailyCostLimitEnabled = !dailyCostLimitEnabled"
                :class="[
                  'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
                  dailyCostLimitEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
                ]"
              >
                <span
                  :class="[
                    'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                    dailyCostLimitEnabled ? 'translate-x-5' : 'translate-x-0'
                  ]"
                />
              </button>
            </div>
            <div v-if="dailyCostLimitEnabled">
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400">$</span>
                <input
                  ...
                  step="1"
                  class="input pl-7"
                  :placeholder="t('admin.accounts.dailyLimits.cost.placeholder')"
                />
              </div>
              <p class="input-hint">{{ t('admin.accounts.dailyLimits.cost.hint') }}</p>
            </div>
          </div>
          ...
          <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
            <div class="mb-3 flex items-center justify-between">
              <label class="input-label mb-0">{{ t('admin.accounts.dailyLimits.requests.label') }}</label>
              <button
                type="button"
                @click="dailyRequestLimitEnabled = !dailyRequestLimitEnabled"
                :class="[
                  'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
                  dailyRequestLimitEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
                ]"
              >
                <span
                  :class="[
                    'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                    dailyRequestLimitEnabled ? 'translate-x-5' : 'translate-x-0'
                  ]"
                />
              </button>
            </div>
            <div v-if="dailyRequestLimitEnabled">
              <input
                ...
                step="1"
                class="input"
                :placeholder="t('admin.accounts.dailyLimits.requests.placeholder')"
              />
              <p class="input-hint">{{ t('admin.accounts.dailyLimits.requests.hint') }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-else class="space-y-5">
      ...
      <!-- Kiro: Custom refresh token form (no OAuth flow needed) -->
      <div v-if="form.platform === 'kiro'">
        <div class="space-y-4">
          <!-- Auth Type Selection -->
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {{ t('admin.accounts.oauth.kiro.authType') }}
            </label>
            <div class="flex gap-2">
              <button
                ...
                :class="[
                  (form.credentials.auth_type || 'Social') === 'Social'
                    ? 'border-rose-500 bg-rose-50 dark:bg-rose-900/20 text-rose-700 dark:text-rose-300'
                    : 'border-gray-200 dark:border-gray-600 hover:border-gray-300'
                ]"
                @click="form.credentials.auth_type = 'Social'"
              >Social</button>
              <button
                type="button"
                class="px-3 py-1.5 rounded-md text-sm border transition-colors"
                :class="[
                  form.credentials.auth_type === 'IdC'
                    ? 'border-rose-500 bg-rose-50 dark:bg-rose-900/20 text-rose-700 dark:text-rose-300'
                    : 'border-gray-200 dark:border-gray-600 hover:border-gray-300'
                ]"
                @click="form.credentials.auth_type = 'IdC'"
              >IdC</button>
            </div>
          </div>
          <!-- Refresh Token -->
          <div>
            ...
            <textarea
              ...
              @input="handleKiroTokenInput"
              :placeholder="t('admin.accounts.oauth.kiro.refreshTokenPlaceholder')"
              rows="3"
            ...
            />
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.oauth.kiro.refreshTokenHint') }}
            </p>
          </div>
          <!-- IdC specific fields -->
          <div v-if="form.credentials.auth_type === 'IdC'" class="space-y-3">
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Client ID</label>
              <input
                :value="(form.credentials.client_id as string) ?? ''"
                @input="form.credentials.client_id = ($event.target as HTMLInputElement).value"
                type="text"
            ...
            <div>
              ...
              <input
                :value="(form.credentials.client_secret as string) ?? ''"
                @input="form.credentials.client_secret = ($event.target as HTMLInputElement).value"
                type="password"
                class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm"
              />
            </div>
          ...
          <!-- Error display -->
          <div v-if="kiroError" class="text-sm text-red-600 dark:text-red-400">{{ kiroError }}</div>
          <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.oauth.kiro.title') }}</span>
        </div>
      </div>
      ...
      <OAuthAuthorizationFlow
        ...
        :show-refresh-token-option="form.platform === 'openai' || form.platform === 'sora' || form.platform === 'antigravity' || form.platform === 'qwen' || form.platform === 'kimi'"
        :show-session-token-option="form.platform === 'sora'"
        :show-access-token-option="form.platform === 'sora'"
        :platform="form.platform"
        :show-project-id="geminiOAuthType === 'code_assist'"
        :qwen-device-flow-info="qwenOAuth.deviceFlowInfo.value"
        :qwen-polling-status="qwenOAuth.deviceFlowStatus.value"
        :kimi-device-flow-info="kimiOAuth.deviceFlowInfo.value"
        :kimi-polling-status="kimiOAuth.deviceFlowStatus.value"
        @generate-url="handleGenerateUrl"
        @cookie-auth="handleCookieAuth"
        @validate-refresh-token="handleValidateRefreshToken"
        @validate-session-token="handleValidateSessionToken"
        @import-access-token="handleImportAccessToken"
        @initiate-device-flow="handleQwenInitiateDeviceFlow"
        @poll-token="handleQwenPollToken"
        @kimi-initiate-device-flow="handleKimiInitiateDeviceFlow"
        @kimi-poll-token="handleKimiPollToken"
        @iflow-cookie-auth="handleIFlowCookieAuth"
      />
    ...
    <template #footer>
      ...
      <div v-else class="flex justify-between gap-3">
        ...
        <!-- Kiro: Validate & Create button -->
        <button
          v-if="form.platform === 'kiro'"
          type="button"
          :disabled="!form.credentials.refresh_token || kiroLoading"
          class="btn btn-primary"
          @click="handleKiroSubmit"
        >
          <svg
            v-if="kiroLoading"
            class="-ml-1 mr-2 h-4 w-4 animate-spin"
          ...
          {{
            kiroLoading
              ? t('admin.accounts.oauth.kiro.validating')
              : t('admin.accounts.oauth.kiro.validateAndCreate')
          }}
        </button>
        <button
...
import { useIFlowOAuth } from '@/composables/useIFlowOAuth'
import { refreshKiroToken } from '@/api/admin/kiro'
import type {
  Proxy,
  AdminGroup,
  AccountPlatform,
...
const oauthStepTitle = computed(() => {
  ...
  if (form.platform === 'iflow') return t('admin.accounts.oauth.iflow.title')
...
const qwenOAuth = useQwenOAuth() // For Qwen OAuth
const kimiOAuth = useKimiOAuth() // For Kimi OAuth
const iflowOAuth = useIFlowOAuth() // For iFlow Cookie Auth

// Kiro state (simple refresh token flow, no composable needed)
const kiroLoading = ref(false)
const kiroError = ref('')

const resolveKiroAuthType = (item: any): string => {
  const authMethod = String(item?.auth_method || item?.authMethod || item?.auth_type || item?.authType || '').toLowerCase()
  const provider = String(item?.provider || item?.provider_type || item?.providerType || '').toLowerCase()
  if (
    authMethod === 'idc' ||
    authMethod === 'idc_custom' ||
...
// Handle Kiro token input - supports both plain token and JSON format
const handleKiroTokenInput = (event: Event) => {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      first = parsed.find((item) => item && typeof item === 'object')
    } else if (parsed && typeof parsed === 'object' && Array.isArray(parsed.accounts)) {
      first = parsed.accounts.find((item: any) => item && typeof item === 'object')
    } else {
      first = parsed
    }
    if (first && typeof first === 'object') {
      const source = first.credentials && typeof first.credentials === 'object' ? first.credentials : first
      const refreshToken =
        ...
        ''
      const clientID = source.client_id || source.clientId || source?.token?.client_id || source?.token?.clientId
      const clientSecret = source.client_secret || source.clientSecret || source?.token?.client_secret || source?.token?.clientSecret
      const region = source.region || source.auth_region || source.authRegion
      const authType = resolveKiroAuthType(source)

      if (refreshToken) {
        form.credentials.refresh_token = refreshToken
      }
      if (clientID) form.credentials.client_id = clientID
      if (clientSecret) form.credentials.client_secret = clientSecret
      if (region) form.credentials.region = region
      if (authType) form.credentials.auth_type = authType
    ...
    // JSON 但没有可识别 token，回退为原始输入内容
    if (value) {
      form.credentials.refresh_token = value
      kiroError.value = ''
      return
    }
  } catch {
...
const currentAuthUrl = computed(() => {
  ...
  if (form.platform === 'qwen') return '' // Qwen uses device flow, no auth URL
  if (form.platform === 'kimi') return '' // Kimi uses device flow
  if (form.platform === 'iflow') return '' // iFlow uses cookie auth
  if (form.platform === 'kiro') return '' // Kiro uses refresh token
...
const currentSessionId = computed(() => {
  ...
  if (form.platform === 'qwen') return qwenOAuth.sessionId.value
  if (form.platform === 'kimi') return kimiOAuth.sessionId.value
  if (form.platform === 'iflow') return ''
...
const currentOAuthLoading = computed(() => {
  ...
  if (form.platform === 'qwen') return qwenOAuth.loading.value
  if (form.platform === 'kimi') return kimiOAuth.loading.value
  if (form.platform === 'iflow') return iflowOAuth.loading.value
...
const currentOAuthError = computed(() => {
  ...
  if (form.platform === 'qwen') return qwenOAuth.error.value
  if (form.platform === 'kimi') return kimiOAuth.error.value
  if (form.platform === 'iflow') return iflowOAuth.error.value
...
const qwenAccountType = ref<'oauth' | 'apikey'>('apikey') // For qwen: oauth or apikey
...
// Daily limits state (all platforms)
const dailyCostLimitEnabled = ref(false)
const dailyCostLimit = ref<number | null>(null)
const dailyRequestLimitEnabled = ref(false)
const dailyRequestLimit = ref<number | null>(null)
...
const isOAuthFlow = computed(() => {
  ...
  // Qwen apikey 类型不需要 OAuth 流程
  if (form.platform === 'qwen' && qwenAccountType.value === 'apikey') {
    return false
  }
  // Kimi: always OAuth flow
  if (form.platform === 'kimi') {
    return true
  }
  // iFlow: always cookie auth flow
  if (form.platform === 'iflow') {
    return true
  }
  // Kiro: always refresh token flow
...
const canExchangeCode = computed(() => {
  // Qwen uses device flow, not exchange code
  if (form.platform === 'qwen') return false
  // Kimi uses device flow, not exchange code
  if (form.platform === 'kimi') return false
  // iFlow uses cookie auth, not exchange code
  if (form.platform === 'iflow') return false
  // Kiro uses refresh token, not exchange code
  if (form.platform === 'kiro') return false
...
watch(
  [accountCategory, addMethod, antigravityAccountType, soraAccountType, qwenAccountType],
  ([category, method, agType, soraType, qwType]) => {
    // Kimi: always OAuth
    if (form.platform === 'kimi') {
      form.type = 'oauth'
      return
    ...
    // Qwen apikey 类型
    if (form.platform === 'qwen' && qwType === 'apikey') {
      form.type = 'apikey'
      return
    }
...
const handleGenerateUrl = async () => {
  ...
  } else if (form.platform === 'qwen') {
    // Qwen uses device flow instead of auth URL
    await handleQwenInitiateDeviceFlow()
  } else if (form.platform === 'kimi') {
    await handleKimiInitiateDeviceFlow()
  } else {
...
const createAccountAndFinish = async (
  ...
  // Merge daily limits into extra (all platforms)
  const mergedExtra: Record<string, unknown> = { ...(extra || {}) }
  if (dailyCostLimitEnabled.value && dailyCostLimit.value != null && dailyCostLimit.value > 0) {
    mergedExtra.daily_cost_limit = dailyCostLimit.value
  }
  if (dailyRequestLimitEnabled.value && dailyRequestLimit.value != null && dailyRequestLimit.value > 0) {
    mergedExtra.daily_request_limit = dailyRequestLimit.value
  }
  await doCreateAccount({
    name: form.name,
    notes: form.notes,
    ...
    type,
    credentials,
    extra: Object.keys(mergedExtra).length > 0 ? mergedExtra : extra,
    proxy_id: form.proxy_id,
    concurrency: form.concurrency,
    priority: form.priority,
    rate_multiplier: form.rate_multiplier,
...
// Qwen OAuth 设备流启动
const handleQwenInitiateDeviceFlow = async () => {
  await qwenOAuth.initiateDeviceFlow(form.proxy_id)
}

// Qwen OAuth 轮询 token
const handleQwenPollToken = async () => {
  const tokenInfo = await qwenOAuth.pollForToken()
  if (!tokenInfo) return

  const credentials = qwenOAuth.buildCredentials(tokenInfo)

  await createAccountAndFinish('qwen', 'oauth', credentials as Record<string, unknown>)
}

// Qwen OAuth Refresh Token 验证（批量）
const handleQwenValidateRT = async (refreshTokenInput: string) => {
  if (!refreshTokenInput.trim()) return
  ...
  if (refreshTokens.length === 0) {
    qwenOAuth.error.value = t('admin.accounts.oauth.qwen.pleaseEnterRefreshToken')
    return
  }

  qwenOAuth.loading.value = true
  qwenOAuth.error.value = ''
  ...
  try {
    for (let i = 0; i < refreshTokens.length; i++) {
      try {
        const tokenInfo = await qwenOAuth.validateRefreshToken(
          refreshTokens[i],
          form.proxy_id
        )
        if (!tokenInfo) {
          failedCount++
          errors.push(`#${i + 1}: ${qwenOAuth.error.value || 'Validation failed'}`)
          qwenOAuth.error.value = ''
          continue
        }
    ...
    if (successCount > 0 && failedCount === 0) {
      ...
      qwenOAuth.error.value = errors.join('\n')
      emit('created')
    } else {
      qwenOAuth.error.value = errors.join('\n')
      appStore.showError(t('admin.accounts.oauth.batchFailed'))
    ...
    qwenOAuth.loading.value = false
  }
}

// Kimi OAuth 设备流启动
const handleKimiInitiateDeviceFlow = async () => {
  await kimiOAuth.initiateDeviceFlow(form.proxy_id)
}

// Kimi OAuth 轮询 token
const handleKimiPollToken = async () => {
  const tokenInfo = await kimiOAuth.pollForToken()
  if (!tokenInfo) return

  const credentials = kimiOAuth.buildCredentials(tokenInfo)

  await createAccountAndFinish('kimi', 'oauth', credentials as Record<string, unknown>)
}

// Kimi OAuth Refresh Token 验证（批量）
const handleKimiValidateRT = async (refreshTokenInput: string) => {
  ...
  try {
    for (let i = 0; i < refreshTokens.length; i++) {
      try {
        const tokenInfo = await kimiOAuth.validateRefreshToken(
          refreshTokens[i],
          form.proxy_id
        )
        if (!tokenInfo) {
...
// iFlow Cookie 认证（批量）
const handleIFlowCookieAuth = async (cookieInput: string) => {
  ...
  if (cookies.length === 0) {
    iflowOAuth.error.value = t('admin.accounts.oauth.iflow.pleaseEnterCookie')
    return
  }

  iflowOAuth.loading.value = true
  iflowOAuth.error.value = ''
  ...
  try {
    for (let i = 0; i < cookies.length; i++) {
      try {
        const tokenInfo = await iflowOAuth.authenticateWithCookie(
          cookies[i],
          form.proxy_id
        )
        if (!tokenInfo) {
          failedCount++
          errors.push(`#${i + 1}: ${iflowOAuth.error.value || 'Authentication failed'}`)
          iflowOAuth.error.value = ''
          continue
        }

        const credentials = iflowOAuth.buildCredentials(tokenInfo)
        const accountName = cookies.length > 1 ? `${form.name} #${i + 1}` : form.name
    ...
    if (successCount > 0 && failedCount === 0) {
      ...
      iflowOAuth.error.value = errors.join('\n')
      emit('created')
    } else {
      iflowOAuth.error.value = errors.join('\n')
      appStore.showError(t('admin.accounts.oauth.batchFailed'))
    ...
    iflowOAuth.loading.value = false
  }
}

// Parse Kiro JSON formats into account entries
// Supports:
// 1. Antigravity-Manager format (snake_case): { provider_type, auth_method, refresh_token, client_id, client_secret, ... }
// 2. Kiro native format (camelCase): { refreshToken, clientId, clientSecret, provider, ... }
interface KiroAccountEntry {
...
const parseCompactKiroLine = (line: string): KiroAccountEntry | null => {
  const parts = line
    .split('----')
    .map((p) => p.trim())
    .filter((p) => p.length > 0)
  ...
  // 1) refresh_token----client_id----client_secret----region
  // 2) access_token----refresh_token----client_id----client_secret----region
  if (parts.length < 4) return null

  const refreshIndex = parts.findIndex((p) => p.startsWith('aor') || p.startsWith('Atzr|') || p.startsWith('atzr|'))
  const accessIndex = parts.findIndex((p) => p.startsWith('aoa'))
  const secretIndex = parts.findIndex((p) => p.split('.').length >= 3)
  const regionRegex = /^[a-z]{2}-[a-z0-9-]+-\d+$/
  const regionIndex = parts.findIndex((p) => regionRegex.test(p))

  const refreshToken = refreshIndex >= 0
    ? parts[refreshIndex]
    : (parts.length === 4 ? parts[0] : parts[1])
  const clientSecret = secretIndex >= 0 ? parts[secretIndex] : parts[parts.length - 2]
  const region = regionIndex >= 0 ? parts[regionIndex] : parts[parts.length - 1]

  const reserved = new Set<number>([refreshIndex, accessIndex, secretIndex, regionIndex].filter((i) => i >= 0))
  let clientID = ''
  for (let i = 0; i < parts.length; i++) {
    if (reserved.has(i)) continue
    if (parts[i] === refreshToken || parts[i] === clientSecret || parts[i] === region) continue
    clientID = parts[i]
    break
  }
...
const parseKiroInput = (input: string): KiroAccountEntry[] | null => {
  ...
  try {
    ...
    // Check if it's a recognized Kiro JSON format
    const isKnownFormat = data.every((item: any) =>
      item.provider_type ||
      item.providerType ||
      item.provider ||
      item.auth_method ||
      ...
      item?.credentials?.refreshToken ||
      item?.token?.refresh_token ||
      item?.token?.refreshToken
    )
    if (!isKnownFormat) return null

    return data.map((item: any) => {
      const source = item?.credentials && typeof item.credentials === 'object' ? item.credentials : item
      // Resolve refresh_token (snake_case or camelCase)
      const refreshToken = source.refresh_token || source.refreshToken || source?.token?.refresh_token || source?.token?.refreshToken || ''
      const authType = resolveKiroAuthType(source)

      return {
        refresh_token: refreshToken,
        auth_type: authType,
        client_id: source.client_id || source.clientId || source?.token?.client_id || source?.token?.clientId || undefined,
        client_secret: source.client_secret || source.clientSecret || source?.token?.client_secret || source?.token?.clientSecret || undefined,
        region: source.region || source.auth_region || source.authRegion || undefined,
        name: item.name || item.email || source.name || undefined,
      }
    }).filter((e: KiroAccountEntry) => e.refresh_token)
  } catch {
    return null
  }
}

// Kiro Refresh Token 验证（批量，支持 JSON 和纯文本）
const handleKiroSubmit = async () => {
  const refreshTokenInput = form.credentials.refresh_token as string || ''
  if (!refreshTokenInput.trim()) {
    kiroError.value = t('admin.accounts.oauth.kiro.pleaseEnterRefreshToken')
    return
  }

  // Try to parse as Antigravity-Manager JSON first
  const jsonEntries = parseKiroInput(refreshTokenInput)

  // Build account entries list
  let entries: KiroAccountEntry[]
  if (jsonEntries && jsonEntries.length > 0) {
    entries = jsonEntries
  } else {
    // Fallback: treat as plain refresh tokens (one per line)
    const refreshTokens = refreshTokenInput
      .split('\n')
    ...
    entries = refreshTokens.map((line) => {
      const compact = parseCompactKiroLine(line)
      if (compact) return compact
      return {
        refresh_token: line,
        auth_type: (form.credentials.auth_type as string) || 'Social',
        client_id: form.credentials.auth_type === 'IdC' ? (form.credentials.client_id as string) : undefined,
        client_secret: form.credentials.auth_type === 'IdC' ? (form.credentials.client_secret as string) : undefined,
      }
    })
  }
  ...
  try {
    for (let i = 0; i < entries.length; i++) {
      ...
      try {
        const tokenInfo = await refreshKiroToken({
          refresh_token: entry.refresh_token,
          auth_type: entry.auth_type,
          client_id: entry.auth_type === 'IdC' ? entry.client_id : undefined,
          client_secret: entry.auth_type === 'IdC' ? entry.client_secret : undefined,
          region: entry.region,
          proxy_id: form.proxy_id
        })
        ...
        const credentials: Record<string, unknown> = {
          ...
          refresh_token: tokenInfo.refresh_token || entry.refresh_token,
          auth_type: entry.auth_type,
          expires_at: tokenInfo.expires_at
        }
        if (entry.auth_type === 'IdC') {
          credentials.client_id = entry.client_id
          credentials.client_secret = entry.client_secret
        }
        if (entry.region) {
        ...
        const accountName = entry.name
          ? entry.name
          : entries.length > 1 ? `${form.name} #${i + 1}` : form.name
