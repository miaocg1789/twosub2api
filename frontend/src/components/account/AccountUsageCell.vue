<template>
  <div v-if="showUsageWindows">
    ...
    <!-- Kiro OAuth accounts: show subscription type + quota -->
    <template v-else-if="account.platform === 'kiro' && account.type === 'oauth'">
      <!-- Subscription type badge -->
      <div v-if="kiroSubscriptionLabel" class="mb-1 flex items-center gap-1">
        <span
          :class="[
            'inline-block rounded px-1.5 py-0.5 text-[10px] font-medium',
      ...
      <div class="space-y-1">
        ...
        <div v-else-if="hasKiroSubscriptionQuota" class="space-y-1">
          <UsageProgressBar
            :label="t('keys.quota')"
            :utilization="kiroQuotaPercent"
            :resets-at="kiroQuotaResetAt"
            color="indigo"
          />
...
const kiroSubscriptionRaw = computed(() => {
  return (usageInfo.value?.kiro_subscription?.subscription_type || '').toUpperCase()
})

const kiroSubscriptionLabel = computed(() => {
  switch (kiroSubscriptionRaw.value) {
    case 'PRO':
...
const hasKiroSubscriptionQuota = computed(() => {
  return (usageInfo.value?.kiro_subscription?.usage_limit || 0) > 0
})

const kiroQuotaPercent = computed(() => {
  const percent = usageInfo.value?.kiro_subscription?.usage_percent || 0
  return Number.isFinite(percent) ? percent : 0
})

const kiroQuotaResetAt = computed(() => {
  const d = usageInfo.value?.kiro_subscription?.next_reset_date
  if (!d) return null
  return `${d}T00:00:00Z`
})

const kiroQuotaText = computed(() => {
  const sub = usageInfo.value?.kiro_subscription
  if (!sub || sub.usage_limit <= 0) return t('admin.accounts.usageWindow.kiroNoQuota')
  return t('admin.accounts.usageWindow.kiroQuota', {
    used: sub.current_usage.toFixed(2),
    limit: sub.usage_limit.toFixed(2),
    percent: kiroQuotaPercent.value.toFixed(1),
  })
})
