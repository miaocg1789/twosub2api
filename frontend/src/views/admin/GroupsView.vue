      ...
      <template #table>
        <DataTable :columns="columns" :data="groups" :loading="loading">
          ...
          <template #cell-rate_multiplier="{ value }">
            <span v-if="value === 0" class="badge badge-green">{{ t('admin.groups.free') }}</span>
            <span v-else class="text-sm text-gray-700 dark:text-gray-300">{{ value }}x</span>
          </template>
    ...
    >
      <form id="create-group-form" @submit.prevent="handleCreateGroup" class="space-y-5">
        ...
        <div class="border-t pt-4">
          <div class="mb-1.5 flex items-center gap-1">
            ...
            <div class="group relative inline-flex">
              ...
              <div class="pointer-events-none absolute bottom-full left-0 z-50 mb-2 w-80 opacity-0 transition-all duration-200 group-hover:pointer-events-auto group-hover:opacity-100">
                <div class="rounded-lg bg-gray-900 p-3 text-white shadow-lg dark:bg-gray-800">
                  <p class="text-xs leading-relaxed text-gray-300">
                    {{ t('admin.groups.perRequestPricing.desc') }}
                  </p>
                  <div class="absolute -bottom-1.5 left-3 h-3 w-3 rotate-45 bg-gray-900 dark:bg-gray-800"></div>
                </div>
              </div>
          ...
          <div v-if="createForm.per_request_pricing_enabled">
            <label class="input-label">{{ t('admin.groups.perRequestPricing.price') }}</label>
            <input
              ...
              min="0"
              class="input"
              :placeholder="t('admin.groups.perRequestPricing.pricePlaceholder')"
            />
            <p class="input-hint">{{ t('admin.groups.perRequestPricing.priceHint') }}</p>
          </div>
        </div>
    ...
    >
      ...
      >
        ...
        <div class="border-t pt-4">
          ...
          <div v-if="editForm.per_request_pricing_enabled">
            <label class="input-label">{{ t('admin.groups.perRequestPricing.price') }}</label>
            <input
...
const handleCreateGroup = async () => {
  ...
  try {
    // 构建请求数据，包含模型路由配置
    const { sora_storage_quota_gb: createQuotaGb, per_request_pricing_enabled, per_request_price, ...createRest } = createForm
    // 构建按次计费配置：启用时用 {"*": price}，禁用时传 null
    let per_request_pricing: Record<string, number> | null = null
    if (per_request_pricing_enabled && per_request_price != null && per_request_price >= 0) {
      per_request_pricing = { '*': per_request_price }
    }
    const requestData = {
      ...createRest,
...
const handleEdit = async (group: AdminGroup) => {
  ...
  // 初始化按次计费
  const prp = group.per_request_pricing
  if (prp && Object.keys(prp).length > 0) {
    editForm.per_request_pricing_enabled = true
    // 取第一个值作为统一单价（兼容旧数据）
    editForm.per_request_price = Object.values(prp)[0] as number
  } else {
    editForm.per_request_pricing_enabled = false
    editForm.per_request_price = null
  }
  // 加载模型路由规则（异步加载账号名称）
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    const { sora_storage_quota_gb: editQuotaGb, per_request_pricing_enabled: editPrpEnabled, per_request_price: editPrpPrice, ...editRest } = editForm
    // 构建按次计费配置：启用时用 {"*": price}，禁用时传 null
    let editPerRequestPricing: Record<string, number> | null = null
    if (editPrpEnabled && editPrpPrice != null && editPrpPrice >= 0) {
      editPerRequestPricing = { '*': editPrpPrice }
    }
    const payload = {
      ...editRest,
      ...
      per_request_pricing: editPerRequestPricing
    }
