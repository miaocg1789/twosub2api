...
type Group struct {
	...
	// 按次计费配置：模型模式(支持通配符) -> 每次请求固定价格(USD)
	PerRequestPricing map[string]float64 `json:"per_request_pricing,omitempty"`
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldPerRequestPricing:
			values[i] = new([]byte)
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldPerRequestPricing:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field per_request_pricing", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.PerRequestPricing); err != nil {
					return fmt.Errorf("unmarshal field per_request_pricing: %w", err)
				}
			}
		default:
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("per_request_pricing=")
	builder.WriteString(fmt.Sprintf("%v", _m.PerRequestPricing))
	builder.WriteByte(')')
