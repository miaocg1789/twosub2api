...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string) (*AccountSelectionResult, error) {
	...
	if len(accounts) == 0 {
		return nil, fmt.Errorf("no available accounts: group=%d platform=%s schedulable_list_empty", derefGroupID(groupID), platform)
	}
	...
	if len(candidates) == 0 {
		...
		return nil, fmt.Errorf("no available accounts (%s)", summarizeSelectionFailureStats(stats))
	}
				...
				if _, excluded := excludedIDs[accountID]; !excluded {
					...
					if err == nil {
						...
						if !clearSticky && s.isAccountInGroup(account, groupID) && (requestedModel == "" || s.isModelSupportedByAccountWithContext(ctx, account, requestedModel)) && s.isAccountSchedulableForModelSelection(ctx, account, requestedModel) {
							if account.Platform == nativePlatform || account.Platform == PlatformKiro || (account.Platform == PlatformAntigravity && account.IsMixedSchedulingEnabled()) {
								if s.debugModelRoutingEnabled() {
...
func isPlatformFilteredForSelection(acc *Account, platform string, allowMixedScheduling bool) bool {
	...
	if allowMixedScheduling {
		...
		// Kiro accounts are always allowed in mixed scheduling with anthropic
...
func (s *GatewayService) isModelSupportedByAccount(account *Account, requestedModel string) bool {
	...
	// Kiro 账号：与测试接口保持一致，先尝试 Kiro 内置模型映射再判断支持。
	// 这样可避免“测试可通过，但调度阶段因请求模型别名不在 model_mapping key 中而被误判不支持”。
	if account.Platform == PlatformKiro {
		if account.IsModelSupported(requestedModel) {
			return true
		}
		if mapped, ok := kiro.ModelMap[requestedModel]; ok && strings.TrimSpace(mapped) != "" {
			return account.IsModelSupported(strings.TrimSpace(mapped))
		}
		lowerReq := strings.ToLower(strings.TrimSpace(requestedModel))
		if mapped, ok := kiro.ModelMap[lowerReq]; ok && strings.TrimSpace(mapped) != "" {
			return account.IsModelSupported(strings.TrimSpace(mapped))
		}
		return false
	}
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	// Kiro platform: use dedicated Kiro forwarding path
	...
	// 应用模型映射（与调度/测试保持一致）
	mappedModel, mappingSource := s.resolveMappedModelForForward(account, reqModel)
...
// forwardKiro handles forwarding requests to the Kiro/CodeWhisperer API
func (s *GatewayService) forwardKiro(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest, startTime time.Time) (*ForwardResult, error) {
	...
	// Parse the request body as AnthropicRequest
	var anthropicReq kiro.AnthropicRequest
	if err := json.Unmarshal(parsed.Body, &anthropicReq); err != nil {
		return nil, fmt.Errorf("parse kiro request body: %w", err)
	}

	// Apply model mapping
	mappedModel, mappingSource := s.resolveMappedModelForForward(account, anthropicReq.Model)
	if mappedModel != anthropicReq.Model {
		anthropicReq.Model = mappedModel
		logger.LegacyPrintf("service.gateway", "Kiro model mapping: %s -> %s (source=%s)", originalModel, mappedModel, mappingSource)
	}

	// Convert Anthropic request to CodeWhisperer format
	cwReq, err := kiro.ConvertAnthropicToCodeWhisperer(anthropicReq)
	if err != nil {
		return nil, fmt.Errorf("convert to codewhisperer: %w", err)
	}

	// Get access token
	accessToken := account.GetCredential("access_token")
	if accessToken == "" {
		return nil, fmt.Errorf("no access_token found for kiro account")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("marshal codewhisperer request: %w", err)
	}

	// Use the primary endpoint
	ep := kiro.KiroEndpoints[0]

	// Build HTTP request
	httpReq, err := http.NewRequestWithContext(ctx, "POST", ep.URL, bytes.NewReader(reqBody))
	if err != nil {
		return nil, fmt.Errorf("create kiro request: %w", err)
	}

	// Set standard Kiro headers
	machineId := account.GetCredential("machine_id")
	kiro.SetKiroHeaders(httpReq, accessToken, machineId)
	httpReq.Header.Set("X-Amz-Target", ep.AmzTarget)

	logger.LegacyPrintf("service.gateway", "[ForwardKiro] account=%d name=%s model=%s->%s endpoint=%s",
		account.ID, account.Name, originalModel, mappedModel, ep.Name)

	setOpsUpstreamRequestBody(c, reqBody)
	...
	if resp.StatusCode >= 400 {
		respBody, _ := io.ReadAll(io.LimitReader(resp.Body, 2<<20))
		logger.LegacyPrintf("service.gateway", "[ForwardKiro] Upstream error: account=%d status=%d body=%s",
			account.ID, resp.StatusCode, truncateString(string(respBody), 500))
	...
	processor := kiro.NewStreamProcessor()

	// Estimate input tokens from request as fallback
	estimatedInputTokens := kiro.EstimateInputTokens(anthropicReq)
	...
	if parsed.Stream {
		...
		defer close(done)
		kiro.StartPingTicker(c, done)

		result, err := processor.ProcessStream(c, resp, messageID, estimatedInputTokens, originalModel, true)
		if err != nil {
			return nil, fmt.Errorf("kiro stream processing: %w", err)
		}
	...
	result, err := processor.ProcessStream(c, resp, messageID, estimatedInputTokens, originalModel, false)
	if err != nil {
		return nil, fmt.Errorf("kiro non-stream processing: %w", err)
	}

	response := kiro.BuildNonStreamResponse(result, messageID)
...
func (s *GatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string, mimicClaudeCode bool) (*streamingResult, error) {
	...
	processSSEEvent := func(lines []string) ([]string, string, *sseUsagePatch, error) {
		...
		newData := bytes.TrimRight(buf.Bytes(), "\n")
...
func (s *GatewayService) RecordUsage(ctx context.Context, input *RecordUsageInput) error {
	...
	// 按次计费优先（优先级：按次 > media > image > token）
	if apiKey.Group != nil {
		if price, ok := apiKey.Group.GetPerRequestPrice(result.Model); ok {
			cost = s.billingService.CalculatePerRequestCost(price, multiplier)
		}
	}

	// 根据请求类型选择计费方式
	if cost == nil && (result.MediaType == "image" || result.MediaType == "video") {
	...
	} else if cost == nil && result.MediaType == "prompt" {
		cost = &CostBreakdown{}
	} else if cost == nil && result.ImageCount > 0 {
		// 图片生成计费
	...
	// 根据计费类型执行扣费
	if isSubscriptionBilling {
		// 订阅模式：更新订阅用量（使用 ActualCost 考虑倍率后的费用，倍率为0时免费）
	...
	// 检查账号每日限额
	s.checkAccountDailyLimits(ctx, account)
...
// resolveMappedModelForForward applies request model mapping rules used by real forwarding.
// Priority: account mapping -> platform-specific fallback mapping.
func (s *GatewayService) resolveMappedModelForForward(account *Account, reqModel string) (string, string) {
	if account == nil {
		return reqModel, ""
	}
	...
	if account.Platform == PlatformKiro {
		// Kiro OAuth forwarding should keep behavior aligned with account test & scheduler checks.
		accountMapped := account.GetMappedModel(reqModel)
		if accountMapped != reqModel {
			return accountMapped, "account"
		}
		if v, ok := kiro.ModelMap[reqModel]; ok {
			v = strings.TrimSpace(v)
			if v != "" && v != reqModel {
				return v, "kiro.ModelMap"
			}
		}
		lowerReq := strings.ToLower(strings.TrimSpace(reqModel))
		if v, ok := kiro.ModelMap[lowerReq]; ok {
			v = strings.TrimSpace(v)
			if v != "" && v != reqModel {
				return v, "kiro.ModelMap(lowercase)"
			}
		}
	}
...
// checkAccountDailyLimits 检查账号是否达到每日费用/请求次数限制
// 如果达到限制，将账号设为 temp_unschedulable 直到明天零点
func (s *GatewayService) checkAccountDailyLimits(ctx context.Context, account *Account) {
	if !account.HasDailyLimits() {
		return
	}

	costLimit := account.GetDailyCostLimit()
	requestLimit := account.GetDailyRequestLimit()

	stats, err := s.usageLogRepo.GetAccountTodayStats(ctx, account.ID)
	if err != nil {
		logger.LegacyPrintf("service.gateway", "checkAccountDailyLimits: GetAccountTodayStats failed: account=%d err=%v", account.ID, err)
		return
	}
	if stats == nil {
		return
	...
	exceeded := false
	var reason string
	if costLimit > 0 && stats.StandardCost >= costLimit {
		exceeded = true
		reason = fmt.Sprintf("daily_cost_limit exceeded: %.4f >= %.4f", stats.StandardCost, costLimit)
	}
	if requestLimit > 0 && stats.Requests >= requestLimit {
		exceeded = true
		reason = fmt.Sprintf("daily_request_limit exceeded: %d >= %d", stats.Requests, requestLimit)
	}
	...
	// 计算明天零点
	tomorrow := timezone.StartOfDay(timezone.Now().Add(24 * time.Hour))

	if err := s.accountRepo.SetTempUnschedulableIfLater(ctx, account.ID, tomorrow, reason); err != nil {
		logger.LegacyPrintf("service.gateway", "checkAccountDailyLimits: SetTempUnschedulableIfLater failed: account=%d err=%v", account.ID, err)
		return
	}
	logger.LegacyPrintf("service.gateway", "checkAccountDailyLimits: account=%d paused until %s reason=%s", account.ID, tomorrow.Format(time.RFC3339), reason)
}
