...
// ModelPricingDisplayItem 用于前端模型广场展示的定价项
type ModelPricingDisplayItem struct {
	ID               string  `json:"id"`
	Provider         string  `json:"provider"`
	InputPrice       float64 `json:"input_price"`        // per 1M tokens
	OutputPrice      float64 `json:"output_price"`       // per 1M tokens
	CacheReadPrice   *float64 `json:"cache_read_price"`  // per 1M tokens, nullable
	CacheCreatePrice *float64 `json:"cache_create_price"` // per 1M tokens, nullable
	Mode             string  `json:"mode"`               // chat, embedding, image_generation, etc.
}

// GetAllModelPricing 返回所有模型定价（供前端模型广场展示）
func (s *PricingService) GetAllModelPricing() ([]ModelPricingDisplayItem, time.Time) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	items := make([]ModelPricingDisplayItem, 0, len(s.pricingData))
	for modelID, p := range s.pricingData {
		// 跳过没有价格的模型
		if p.InputCostPerToken == 0 && p.OutputCostPerToken == 0 && p.OutputCostPerImage == 0 {
			continue
		}

		provider := normalizeProviderName(p.LiteLLMProvider)

		item := ModelPricingDisplayItem{
			ID:          modelID,
			Provider:    provider,
			InputPrice:  p.InputCostPerToken * 1_000_000,
			OutputPrice: p.OutputCostPerToken * 1_000_000,
			Mode:        p.Mode,
		}

		if p.CacheReadInputTokenCost > 0 {
			v := p.CacheReadInputTokenCost * 1_000_000
			item.CacheReadPrice = &v
		}
		if p.CacheCreationInputTokenCost > 0 {
			v := p.CacheCreationInputTokenCost * 1_000_000
			item.CacheCreatePrice = &v
		}

		items = append(items, item)
	}

	// 按 provider 然后按 model id 排序
	...
	return items, s.lastUpdated
}

// normalizeProviderName 标准化提供商名称
func normalizeProviderName(litellmProvider string) string {
	p := strings.ToLower(litellmProvider)
	switch {
	...
	case strings.Contains(p, "cohere"):
		return "Cohere"
	case strings.Contains(p, "qwen"), strings.Contains(p, "alibaba"):
		return "Qwen"
	case strings.Contains(p, "zhipu"), strings.Contains(p, "glm"):
		return "GLM"
	default:
		if litellmProvider != "" {
			// 首字母大写
			return strings.ToUpper(litellmProvider[:1]) + litellmProvider[1:]
		}
		return "Other"
	}
}
