...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	if resetTimestamp == "" {
		...
		case PlatformKiro:
			// Kiro 的 429 常见于模型容量不足（如 INSUFFICIENT_MODEL_CAPACITY），
			// 通常是瞬时拥塞，不应直接将整个账号标记为 5 分钟不可调度。
			// 否则单账号分组会被全部过滤并返回 "no available accounts"。
			slog.Warn("kiro_429_without_reset_header", "account_id", account.ID, "skip_global_ratelimit", true)
			return
		}
