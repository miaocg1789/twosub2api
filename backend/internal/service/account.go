...
//   - Unix 秒时间戳: "1735689600" / 1735689600
//   - Unix 毫秒时间戳: "1735689600000" / 1735689600000
//   - Unix 微秒时间戳: "1735689600000000" / 1735689600000000
//   - Unix 纳秒时间戳: "1735689600000000000" / 1735689600000000000
func (a *Account) GetCredentialAsTime(key string) *time.Time {
	...
	// 尝试 Unix 时间戳（纯数字字符串），自动识别秒/毫秒/微秒/纳秒。
	if ts, err := strconv.ParseInt(s, 10, 64); err == nil {
		t := unixTimestampToTime(ts)
		return &t
	}
	return nil
}
...
func (a *Account) IsIFlow() bool {
	return a.Platform == PlatformIFlow
}
...
// IsOpenAICompatible returns true for platforms that use the OpenAI-compatible API format.
func (a *Account) IsOpenAICompatible() bool {
	switch a.Platform {
	case PlatformOpenAI, PlatformDeepSeek, PlatformQwen, PlatformGLM, PlatformKimi, PlatformIFlow:
		return true
	}
	return false
}
...
// GetDailyRequestLimit 获取每日请求次数限制
// 返回 0 表示不限制
func (a *Account) GetDailyRequestLimit() int64 {
	if a.Extra == nil {
		return 0
	}
	if v, ok := a.Extra["daily_request_limit"]; ok {
		return int64(parseExtraInt(v))
	}
	return 0
}

// HasDailyLimits 检查是否启用了每日限制（费用或次数任一 > 0）
func (a *Account) HasDailyLimits() bool {
	return a.GetDailyCostLimit() > 0 || a.GetDailyRequestLimit() > 0
}
