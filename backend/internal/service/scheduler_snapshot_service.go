...
func (s *SchedulerSnapshotService) runInitialRebuild() {
	...
	// Always merge with defaultBuckets so newly added groups/platforms are discovered
	defaults, defErr := s.defaultBuckets(ctx)
	if defErr != nil {
		logger.LegacyPrintf("service.scheduler_snapshot", "[Scheduler] default buckets failed: %v", defErr)
	}
	if len(defaults) > 0 {
