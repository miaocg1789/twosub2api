...
type Group struct {
	...
	// 按次计费配置
	// key: 模型匹配模式（支持 * 通配符）, value: 每次请求固定价格(USD)
	PerRequestPricing map[string]float64
...
// GetPerRequestPrice 获取按次计费价格
// 只要分组配置了按次计费，所有请求统一按固定价格计费
func (g *Group) GetPerRequestPrice(model string) (float64, bool) {
	if len(g.PerRequestPricing) == 0 {
		return 0, false
	}

	// 全局通配符 * 优先（简化模式：统一单价）
	if price, ok := g.PerRequestPricing["*"]; ok {
		return price, true
	}

	// 兼容旧数据：精确匹配
	if model != "" {
		if price, ok := g.PerRequestPricing[model]; ok {
			return price, true
		}
	}
	...
	// 兼容旧数据：通配符前缀匹配
	if model != "" {
		for pattern, price := range g.PerRequestPricing {
	...
	// 如果只有一个配置项，直接使用（兜底）
	if len(g.PerRequestPricing) == 1 {
		for _, price := range g.PerRequestPricing {
			return price, true
		}
	}
