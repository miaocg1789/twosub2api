...
func TestClaudeTokenRefresher_NeedsRefresh(t *testing.T) {
	...
	now := time.Now()
	farFutureMillis := now.Add(2 * time.Hour).UnixMilli()
	...
	tests := []struct {
		...
		{
			name: "expires_at as milliseconds - far future",
			credentials: map[string]any{
				"expires_at": farFutureMillis, // 毫秒时间戳
			},
			wantRefresh: false,
		},
		{
...
func TestClaudeTokenRefresher_NeedsRefresh_WithinWindow(t *testing.T) {
	...
	expiresAtMillis := time.Now().Add(15 * time.Minute).UnixMilli()
	...
	tests := []struct {
		...
		{
			name: "int64 milliseconds - within refresh window",
			credentials: map[string]any{
...
func TestClaudeTokenRefresher_NeedsRefresh_OutsideWindow(t *testing.T) {
	...
	expiresAtMillis := time.Now().Add(1 * time.Hour).UnixMilli()
	...
	tests := []struct {
		...
		{
			name: "int64 milliseconds - outside refresh window",
			credentials: map[string]any{
				"expires_at": expiresAtMillis,
			},
		},
