...
// CalculatePerRequestCost 计算按次计费费用
// price: 每次请求的固定价格（USD）
// rateMultiplier: 费率倍数
func (s *BillingService) CalculatePerRequestCost(price float64, rateMultiplier float64) *CostBreakdown {
	if rateMultiplier < 0 {
		rateMultiplier = 1.0
	}
	return &CostBreakdown{
		TotalCost:  price,
		ActualCost: price * rateMultiplier,
	}
}
