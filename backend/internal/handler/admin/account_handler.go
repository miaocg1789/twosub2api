...
type AccountHandler struct {
	...
	accountBalanceService   *service.AccountBalanceService
...
type AccountWithConcurrency struct {
	*dto.Account
	...
	// 每日限额相关字段，仅在启用每日限制时返回
	DailyCostLimit    *float64 `json:"daily_cost_limit,omitempty"`
	DailyRequestLimit *int64   `json:"daily_request_limit,omitempty"`
	DailyUsedCost     *float64 `json:"daily_used_cost,omitempty"`
	DailyUsedRequests *int64   `json:"daily_used_requests,omitempty"`
}
...
func (h *AccountHandler) buildAccountResponseWithRuntime(ctx context.Context, account *service.Account) AccountWithConcurrency {
	...
	if account.HasDailyLimits() {
		...
		if costLimit > 0 {
			item.DailyCostLimit = &costLimit
		}
		if requestLimit > 0 {
			item.DailyRequestLimit = &requestLimit
		}
		if h.accountUsageService != nil {
			if stats, err := h.accountUsageService.GetTodayStats(ctx, account.ID); err == nil && stats != nil {
				usedCost := stats.StandardCost
				usedRequests := stats.Requests
				item.DailyUsedCost = &usedCost
				item.DailyUsedRequests = &usedRequests
			}
		}
	}
...
// mergeDailyLimitsIntoExtra 将 daily_cost_limit 和 daily_request_limit 合并到 extra map
func mergeDailyLimitsIntoExtra(extra map[string]any, dailyCostLimit *float64, dailyRequestLimit *int64) map[string]any {
	if dailyCostLimit == nil && dailyRequestLimit == nil {
		return extra
	}
	if extra == nil {
		extra = make(map[string]any)
	}
	if dailyCostLimit != nil {
		extra["daily_cost_limit"] = *dailyCostLimit
	}
	if dailyRequestLimit != nil {
		extra["daily_request_limit"] = *dailyRequestLimit
	}
	return extra
}
...
func (h *AccountHandler) List(c *gin.Context) {
	...
	dailyLimitAccountIDs := make([]int64, 0)             // 有每日限制的账号
	dailyLimitConfigs := make(map[int64][2]float64)      // accountID -> [costLimit, requestLimit]
	for i := range accounts {
		...
		if acc.HasDailyLimits() {
			dailyLimitAccountIDs = append(dailyLimitAccountIDs, acc.ID)
			dailyLimitConfigs[acc.ID] = [2]float64{acc.GetDailyCostLimit(), float64(acc.GetDailyRequestLimit())}
		}
	}
	...
	// 获取每日限额统计（批量查询）
	var dailyStats map[int64]*service.WindowStats
	if len(dailyLimitAccountIDs) > 0 && h.accountUsageService != nil {
		dailyStats, _ = h.accountUsageService.GetTodayStatsBatch(c.Request.Context(), dailyLimitAccountIDs)
		if dailyStats == nil {
			dailyStats = make(map[int64]*service.WindowStats)
		}
	}
	...
	for i := range accounts {
		...
		// 添加每日限额信息（仅当启用时）
		if limits, ok := dailyLimitConfigs[acc.ID]; ok {
			if limits[0] > 0 {
				costLimit := limits[0]
				item.DailyCostLimit = &costLimit
			}
			if limits[1] > 0 {
				reqLimit := int64(limits[1])
				item.DailyRequestLimit = &reqLimit
			}
			if dailyStats != nil {
				if stats, ok := dailyStats[acc.ID]; ok && stats != nil {
					usedCost := stats.StandardCost
					usedRequests := stats.Requests
					item.DailyUsedCost = &usedCost
					item.DailyUsedRequests = &usedRequests
...
func (h *AccountHandler) Create(c *gin.Context) {
	...
	result, err := executeAdminIdempotent(c, "admin.accounts.create", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		extra := mergeDailyLimitsIntoExtra(req.Extra, req.DailyCostLimit, req.DailyRequestLimit)
...
func (h *AccountHandler) BulkUpdate(c *gin.Context) {
	...
	hasUpdates := req.Name != "" ||
		...
		len(req.Extra) > 0 ||
		req.DailyCostLimit != nil ||
		req.DailyRequestLimit != nil
...
func (h *AccountHandler) GetAvailableModels(c *gin.Context) {
	...
	if account.Platform == service.PlatformKiro {
		mapping := account.GetModelMapping()
		if len(mapping) == 0 {
			models := make([]claude.Model, 0, len(kiro.DefaultModels))
			for _, id := range kiro.DefaultModels {
				display := id
				if v, ok := kiro.DisplayModelName[id]; ok && strings.TrimSpace(v) != "" {
					display = v
				}
		...
		for requestedModel := range mapping {
			display := requestedModel
			if canonical, ok := kiro.CanonicalModelName[requestedModel]; ok && strings.TrimSpace(canonical) != "" {
				if friendly, ok := kiro.DisplayModelName[canonical]; ok && strings.TrimSpace(friendly) != "" {
					display = friendly
				}
			}
			models = append(models, claude.Model{
...
// GetBalance queries upstream balance for a single account.
...
func (h *AccountHandler) GetBalance(c *gin.Context) {
	...
	info, err := h.accountBalanceService.QueryBalance(c.Request.Context(), accountID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// BatchGetBalance queries upstream balance for multiple accounts.
// POST /api/v1/admin/accounts/batch-balance
func (h *AccountHandler) BatchGetBalance(c *gin.Context) {
	var req struct {
		AccountIDs []int64 `json:"account_ids" binding:"required,min=1"`
	}
	if err := c.ShouldBindJSON(&req); err != nil {
	...
	if len(req.AccountIDs) > 50 {
		response.BadRequest(c, "Maximum 50 accounts per batch request")
		return
	}

	results := h.accountBalanceService.QueryBalanceBatch(c.Request.Context(), req.AccountIDs)
	response.Success(c, results)
}

// FetchUpstreamModels fetches available models from upstream API for the account.
// GET /api/v1/admin/accounts/:id/upstream-models
func (h *AccountHandler) FetchUpstreamModels(c *gin.Context) {
	...
	models, err := h.accountTestService.FetchUpstreamModels(c.Request.Context(), accountID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
