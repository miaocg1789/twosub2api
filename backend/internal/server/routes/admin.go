...
func RegisterAdminRoutes(
	...
	admin.Use(gin.HandlerFunc(adminAuth))
	{
		...
		// iFlow OAuth
		registerIFlowOAuthRoutes(admin, h)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.GET("/:id/upstream-models", h.Admin.Account.FetchUpstreamModels)
		...
		accounts.POST("/batch-balance", h.Admin.Account.BatchGetBalance)
...
func registerQwenOAuthRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	{
		qwen.POST("/oauth/device-flow", h.Admin.QwenOAuth.InitiateDeviceFlow)
		qwen.POST("/oauth/poll-token", h.Admin.QwenOAuth.PollForToken)
		qwen.POST("/oauth/refresh-token", h.Admin.QwenOAuth.RefreshToken)
	}
}
...
func registerKimiOAuthRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	{
		kimi.POST("/oauth/device-flow", h.Admin.KimiOAuth.InitiateDeviceFlow)
		kimi.POST("/oauth/poll-token", h.Admin.KimiOAuth.PollForToken)
		kimi.POST("/oauth/refresh-token", h.Admin.KimiOAuth.RefreshToken)
	}
}

func registerIFlowOAuthRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	iflow := admin.Group("/iflow")
	{
		iflow.POST("/oauth/cookie-auth", h.Admin.IFlowOAuth.AuthenticateWithCookie)
		iflow.POST("/oauth/refresh-key", h.Admin.IFlowOAuth.RefreshAPIKey)
	}
}
